...
def main(user, list_level, list_name, questions):
    ...
    if list_name=="*":
        print("Hello, %s. Today's test will be %d questions at level %d" % (profile.name, questions, list_level)) 
    else:
        print("Hello, %s. Today's test will be %d questions from the %s list(s)" % (profile.name, questions, list_name))
...
def get_random_words(level, name, questions, dictionary, profile):
    ...
    if name == "*":
        jsonpath_filter = '$.dictionary[?(@.level==%d || @.level==%d)].words' % (level, level-1)
    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--level', help='The level (school year) to test at', dest='list_level')
    parser.add_argument('--test', help='The named list of words you want to be tested on', default="*", dest='list_name')
    parser.add_argument('--questions', help='The number of words to test (Default 10)', default=10)
